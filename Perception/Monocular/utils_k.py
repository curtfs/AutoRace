...
vis_path = "/outputs_k/visualization/"
...
def vis_tensor_and_save(image, h, w, tensor_output, image_name, output_uri):
    ...
    for pt in np.array(tensor_output):
        print(pt[0])
        cv2.circle(image, (int(pt[0]*w ), int(pt[1]*h)), 3, colors[i], -1)
        i += 1
...
def vis_tensor_and_save_2(image,x_translation,y_translation, h, w, tensor_output, image_name, output_uri,target_path):
    colors = [(0, 255, 0), (255, 0, 0), (255, 255, 0), (0, 255, 255), (255, 0, 255), (127, 255, 127), (255, 127, 127)]
    i = 0
    for pt in np.array(tensor_output):
        cv2.circle(image, (int(((pt[0]*w)+x_translation) ), int(((pt[1]*h)+y_translation) )), 4, colors[i], -1)
        i += 1
